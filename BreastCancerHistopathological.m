...
% =========================================================================
% BREAST CANCER HISTOPATHOLOGICAL IMAGE CLASSIFICATION Complete 8-Step
% Pattern Recognition Pipeline with Advanced Features - Multiple Feature
% Selection Methods (F-score, RFE, Tree-based, LASSO, PCA) - k-Fold
% Cross-Validation - Statistical Significance Testing - Comprehensive
...
% =========================================================================
% FIRST STEP (Every run): Reset Training_Results folder
% =========================================================================


% =========================================================================
% Prepare experiment output folder (recreate fresh each run)
% =========================================================================
expDir = 'Training_Results';

% Ensure we are not writing into an old run folder
if exist(expDir, 'dir')
    % Best-effort release (should be clean at script start, but Windows can be sticky)
    try close all force; catch, end
    try fclose('all'); catch, end

    % Retry deletion a few times (handles Windows latency)
    for kDel = 1:3
        try
            rmdir(expDir, 's');
            break;
    ...
    % Fallback for stubborn locks on Windows
    if exist(expDir, 'dir')
        try
            system(sprintf('rmdir /S /Q "%s"', fullfile(pwd, expDir)));
        catch
        end
    end
end
...
% Cleanup behaviour after 100X preparation
% If true, delete archive.zip immediately after the 100X prepared dataset is created.
% (BreakHis_Main is deleted after preparation, if possible.)
cfg.deleteDatasetZipAfterPrep = false;  % set to false to keep archive.zip
...
cfg.K_candidates = [150, 300, 400];  % EXPANDED range
...
%% ═══════════════════════════════════════════════════════════════════════
%% ═══════════════════════════════════════════════════════════════════════
%% DATASET BOOTSTRAP (ADDED)
%% - If archive.zip exists next to this .m file -> extract to BreakHis_Main
%% - Else try to download archive.zip from fallback mirrors (A then B)
%% - Resolve the correct raw dataset root (contains benign/malignant)
%% - Show console logs with a simple progress bar
%% ═══════════════════════════════════════════════════════════════════════

fprintf('=================================================================\n');
fprintf('  DATASET BOOTSTRAP (BreakHis)\n');
fprintf('=================================================================\n');

bootstrap = struct();
bootstrap.scriptFullPath = mfilename('fullpath');
bootstrap.scriptDir = fileparts(bootstrap.scriptFullPath);

% Ensure all paths are relative to the script location (not the current folder)
bootstrap.archiveZip = fullfile(bootstrap.scriptDir, 'archive.zip');
bootstrap.extractDir = fullfile(bootstrap.scriptDir, 'BreakHis_Main');

% (Optional) keep original configured raw root for reference
bootstrap.cfgRawRootOriginal = cfg.rawDatasetRoot;

% Force cfg.rawDatasetRoot to the script-local BreakHis_Main first
cfg.rawDatasetRoot = bootstrap.extractDir;

localConsoleProgress(0, 'Checking for existing extracted dataset...');

% Heuristic: if BreakHis_Main already contains PNGs, skip download/extract
alreadyExtracted = isfolder(bootstrap.extractDir) && ~isempty(dir(fullfile(bootstrap.extractDir, '**', '*.png')));

if alreadyExtracted
    localConsoleProgress(100, 'Dataset already present. Skipping download/extract.');
else
    % Need archive.zip (local or downloaded) and then extract
    if exist(bootstrap.archiveZip, 'file')
        localConsoleProgress(20, 'Found local archive.zip.');
    else
        % Local ZIP not found; using fallback mirrors:
        %   1) Try direct mirror URL (if available)
        %   2) Try Google Drive direct download
        %   3) If all fail, instruct manual placement of archive.zip (one-time)
        localConsoleProgress(20, 'archive.zip not found. Attempting fallback downloads...');

        mirrorA = 'http://58.26.41.115:7698/XBRL/Versions/archive.zip';
        mirrorB = 'https://drive.google.com/uc?export=download&id=12BqwzjO9pQyIyxvZhkPK06fl8GmDJ4Mf';

        fprintf('\n');
        fprintf('=================================================================\n');
        fprintf('  BREAKHIS DATASET ACQUISITION\n');
        fprintf('=================================================================\n');
        fprintf('This script will use local archive.zip if present. If not found, it will try\n');
        fprintf('to download archive.zip from fallback mirrors (A then B).\n\n');
        fprintf('Expected location for archive.zip (if you download it manually):\n  %s\n\n', bootstrap.scriptDir);

% Try mirror A
        localConsoleProgress(30, 'Trying fallback mirror A (direct zip)...');
        try
            websave(bootstrap.archiveZip, mirrorA);
        catch
            % ignore and try next
        end

        % Try mirror B (Google Drive)
        if ~exist(bootstrap.archiveZip, 'file')
            localConsoleProgress(45, 'Trying fallback mirror B (Google Drive)...');
            try
                websave(bootstrap.archiveZip, mirrorB);
            catch
                % ignore and fall back to manual instructions
            end
        end

        if ~exist(bootstrap.archiveZip, 'file')
            localConsoleProgress(0, 'Dataset zip still missing.');
            error(['archive.zip is not available.\n\n' ...
                   'Please download the dataset manually (one-time):\n' ...
                                      'Steps:\n' ...
                                                         '  3) Ensure the file name is archive.zip\n' ...
                   '  4) Place it in:\n     %s\n\n' ...
                   'Then re-run this script.\n'], bootstrap.scriptDir);
        end
        localConsoleProgress(60, 'archive.zip ready.');
    end

    % Ensure clean extraction directory
    localConsoleProgress(70, 'Preparing extraction folder...');
    if exist(bootstrap.extractDir, 'dir')
        try rmdir(bootstrap.extractDir, 's'); catch, end
    end
    mkdir(bootstrap.extractDir);

    % Extract archive.zip
    localConsoleProgress(80, 'Extracting archive.zip to BreakHis_Main...');
    try
        unzip(bootstrap.archiveZip, bootstrap.extractDir);
    catch ME
        error('Failed to unzip %s\nReason: %s', bootstrap.archiveZip, ME.message);
    end
    localConsoleProgress(90, 'Extraction complete.');
end

% Resolve the actual BreakHis raw root that contains benign/malignant folders
localConsoleProgress(95, 'Resolving dataset root (benign/malignant)...');
cfg.rawDatasetRoot = localResolveBreakHisRawRoot(bootstrap.extractDir);

localConsoleProgress(100, 'Bootstrap complete.');
fprintf('\n[Bootstrap] Using raw dataset root: %s\n\n', cfg.rawDatasetRoot);
fprintf('=================================================================\n');
...
% ===== IMMEDIATE CLEANUP AFTER 100X PREPARATION (release handles + delete BreakHis_Main) =====
try
    % Ensure we are not inside the folder we are trying to delete
    if exist('bootstrap', 'var') && isfield(bootstrap, 'scriptDir') && isfolder(bootstrap.scriptDir)
        cd(bootstrap.scriptDir);
    end

    % Release common handles that can lock folders on Windows
    try, close all force; catch, end
    try, fclose('all'); catch, end

    % Clear any datastore-like variables (common naming patterns)
    try
        clear imds imdsTrain imdsTest imdsAll ds
        clearvars -regexp '(^imds)|(^ds)'
    catch
    end

    % Windows filesystem latency
    pause(1.5);

    % Delete BreakHis_Main (bootstrap.extractDir)
    if exist('bootstrap', 'var') && isfield(bootstrap, 'extractDir') && isfolder(bootstrap.extractDir)
        for k = 1:3
            try
                rmdir(bootstrap.extractDir, 's');
            catch
                pause(1.5);
            end
            if ~isfolder(bootstrap.extractDir), break; end
        end

        % Fallback for stubborn Windows locks
        if isfolder(bootstrap.extractDir) && ispc
            system(sprintf('rmdir /S /Q "%s"', bootstrap.extractDir));
        end

        if ~isfolder(bootstrap.extractDir)
            fprintf('[Cleanup] Deleted folder after 100X prep: %s\n', bootstrap.extractDir);
        else
            fprintf('[Cleanup] WARNING: Could not delete folder (still locked): %s\n', bootstrap.extractDir);
        end
    end

    % Optionally delete archive.zip immediately after 100X prep
    if isfield(cfg, 'deleteDatasetZipAfterPrep') && cfg.deleteDatasetZipAfterPrep ...
            && exist('bootstrap', 'var') && isfield(bootstrap, 'archiveZip') && isfile(bootstrap.archiveZip)
        for k = 1:3
            try
                delete(bootstrap.archiveZip);
                break;
            catch
                pause(1.0);
            end
        end
        if ~isfile(bootstrap.archiveZip)
            fprintf('[Cleanup] Deleted archive.zip after 100X prep: %s\n', bootstrap.archiveZip);
        else
            fprintf('[Cleanup] WARNING: Could not delete archive.zip (still locked): %s\n', bootstrap.archiveZip);
        end
    end
catch cleanupErr
    fprintf('[Cleanup] WARNING: Cleanup after 100X prep encountered an error: %s\n', cleanupErr.message);
end
% ==============================================================================
...
%% ═══════════════════════════════════════════════════════════════════════
%% CLEANUP (UPDATED)
%% Release dataset handles, then delete BreakHis_Main folder and archive.zip.
%% ═══════════════════════════════════════════════════════════════════════
try
    if exist('bootstrap', 'var')
        % Cache paths first (so we can clear variables safely)
        extractDir = '';
        archiveZip = '';
        scriptDir  = '';
        if isfield(bootstrap, 'extractDir'); extractDir = bootstrap.extractDir; end
        if isfield(bootstrap, 'archiveZip'); archiveZip = bootstrap.archiveZip; end
        if isfield(bootstrap, 'scriptDir');  scriptDir  = bootstrap.scriptDir;  end

        % --- RELEASE HANDLES / LOCKS (important on Windows) ---
        try
            close all force;            % close figures that may hold file refs
        catch
        end
        try
            fclose('all');              % close any open file handles
        catch
        end
        try
            if ~isempty(scriptDir) && isfolder(scriptDir)
                cd(scriptDir);          % ensure we are not inside BreakHis_Main
            end
        catch
        end

        % Clear common datastore / file-handle variables (no changes to your pipeline logic)
        try clear imds imdsAll imdsTrain imdsTest; catch, end
        try clear ds dsAll dsTrain dsTest; catch, end
        try clearvars -regexp '^(imds|ds)'; catch, end

        pause(1.5); % let OS release locks

        % --- DELETE TEMP FOLDER (with retries + OS fallback) ---
        if ~isempty(extractDir) && isfolder(extractDir)
            fprintf('[Cleanup] Deleting folder: %s\n', extractDir);
            deleted = false;
            for k = 1:3
                try
                    rmdir(extractDir, 's');
                    deleted = ~isfolder(extractDir);
                catch
                    deleted = false;
                end
                if deleted, break; end
                pause(1.0);
            end
            if ~deleted && ispc
                % Windows fallback (handles stubborn locks better once released)
                system(sprintf('rmdir /S /Q "%s"', extractDir));
            end
        end

        % --- DELETE ZIP (with retries) ---
        if isfield(cfg,'deleteDatasetZipAfterPrep') && cfg.deleteDatasetZipAfterPrep ...
            && ~isempty(archiveZip) && exist(archiveZip, 'file')
            fprintf('[Cleanup] Deleting file: %s\n', archiveZip);
            for k = 1:3
                try
                    delete(archiveZip);
                catch
                end
                if ~exist(archiveZip, 'file'), break; end
                pause(0.5);
            end
        else
            if ~isempty(archiveZip) && exist(archiveZip, 'file')
                fprintf('[Cleanup] Keeping archive.zip (cfg.deleteDatasetZipAfterPrep = false): %s\n', archiveZip);
            end
        end
    end
catch
...
% --- AUTO-LAUNCH GUI AFTER PIPELINE COMPLETES (ONLY IF TRAINING SUCCEEDED) ---
try
    modelsFile = fullfile(expDir,'models.mat');
    resultsFile = fullfile(expDir,'results.mat');
    logFile    = fullfile(expDir,'training_log.txt');

    trainingSucceeded = isfile(modelsFile) && isfile(resultsFile) && isfile(logFile);

    if trainingSucceeded
        if exist('breastCancerHistopathologyGui','file') == 2
            fprintf('\n[GUI] Training outputs detected. Launching breastCancerHistopathologyGui...\n');
            breastCancerHistopathologyGui();
            % --- Maximize GUI window(s) (figure/uifigure) ---
            pause(0.5);  % allow GUI to initialize
            figs = findall(0,'Type','figure');
            for kk = 1:numel(figs)
                try
                    figs(kk).WindowState = 'maximized';
                catch
                    try
                        set(figs(kk),'Units','normalized','OuterPosition',[0 0 1 1]);
                    catch
                    end
                end
            end
        else
            warning('[GUI] Training outputs detected but breastCancerHistopathologyGui.m was not found on the MATLAB path. Skipping GUI launch.');
        end
    else
        fprintf('\n[GUI] Training outputs not found in "%s". GUI launch skipped.\n', expDir);
        fprintf('[GUI] Expected: models.mat, results.mat, training_log.txt\n');
    end
catch ME
    warning('[GUI] Failed to evaluate GUI launch conditions: %s', ME.message);
end

end % END MAIN

function localConsoleProgress(pct, msg)
% Simple console progress bar (diary-friendly)
% pct: 0..100
% msg: status text
barWidth = 28;
pct = max(0, min(100, round(pct)));
filled = round(barWidth * (pct/100));
bar = [repmat('█', 1, filled), repmat('░', 1, barWidth-filled)];
% \r keeps it on one line in console; diary will capture each update as it appears
fprintf('\r[Bootstrap] [%s] %3d%%  %s', bar, pct, msg);
if pct >= 100
    fprintf('\n');
end
end
...
function rawRoot = localResolveBreakHisRawRoot(extractDir)
% Finds the folder that directly contains "benign" and "malignant".
% Typical BreakHis dataset layout:
%   BreakHis_Main/BreaKHis_v1/histology_slides/breast/{benign,malignant}/...
candidates = {
    extractDir, ...
    fullfile(extractDir, 'BreaKHis_v1', 'histology_slides', 'breast'), ...
    fullfile(extractDir, 'histology_slides', 'breast'), ...
    fullfile(extractDir, 'breast') ...
};

% Also search recursively (bounded) for a 'breast' folder containing the class dirs
try
    breastDirs = dir(fullfile(extractDir, '**', 'breast'));
    for i = 1:numel(breastDirs)
        if breastDirs(i).isdir
            candidates{end+1} = fullfile(breastDirs(i).folder, breastDirs(i).name); %#ok<AGROW>
        end
    end
catch
end
...
for i = 1:numel(candidates)
    p = candidates{i};
    if isfolder(p) && isfolder(fullfile(p,'benign')) && isfolder(fullfile(p,'malignant'))
        rawRoot = p;
        return;
    end
end

error(['Could not locate the BreakHis raw root containing "benign" and "malignant".\n' ...
       'Searched under: %s\n' ...
       'Please check the extracted folder structure.'], extractDir);
end
...
% LASSO regularization - FIXED for categorical input Convert categorical to
% numeric (0 and 1)
